...
\textcolor{blue}{Ao final, lembrar de colocar um resumo (2 linhas) dos resultados aqui.}
...
\textcolor{green}{Eu tiraria essa parte: O foco recai sobre sistemas de recomendação, modelos híbridos, sinais de ranqueamento, aprendizado para ranqueamento e métricas de avaliação, de modo a explicar os principais termos introduzidos nas seções anteriores.}
...
\textcolor{green}{Essa palavra "escore" existe? Acho melhor trocar para score em itálico ou pontuação, não?}

\subsection{Sinais de ranqueamento}

\textcolor{green}{Acredito que seria importante trazer umas 2 ou 3 referências (citações) a mais nessa subseção ao longo dos parágrafos. Existem muitas afirmações que estão sem citações.}
...
\textcolor{green}{Eu tiraria aqui o título da subseção. Acho que esse texto pode ser em sequência, sem a quebra do título, ou seja, sem abrir uma nova subseção. Além disso, eu juntaria esses dois próximos parágrafos em um único.}
...
Esta seção apresenta os materiais empregados no estudo e o método adotado para desenvolver, treinar e comparar os modelos de recomendação. \textcolor{green}{Eu tiraria essa próxima frase (ainda acho ela artificial):} O foco recai sobre o fluxo de dados e sobre as escolhas metodológicas associadas ao sistema de recomendação, que atua como o motor de personalização do aplicativo FitConnect.
...
O método desenvolvido foi organizado em cinco etapas principais: Etapa 1: extração e filtragem do conteúdo; Etapa 2: preparação de dados; Etapa 3: separação experimental; Etapa 4: treinamento dos modelos; e Etapa 5: avaliação e rastreabilidade. A Figura \ref{fig:fluxograma_metodologia} ilustra o fluxo de dados desde a base bruta até a avaliação dos modelos.

\textcolor{green}{Colocar o número da etapa, por exemplo: Etata 1: Extração e Filtragem Fitness. Aí no título da subseção, faz a mesma coisa. Fica mais fácil de achar qual é qual etapa.}
...
\node[box, right=of ldbc] (extracao) {Etapa 1: Extração e\\Filtragem Fitness};
\node[box, right=of extracao] (preparacao) {Etapa 2: Preparação\\de Dados};
\node[box, below=of preparacao] (split) {Etapa 3: Separação\\(Treino/Val/Teste)};
\node[box, left=of split] (treinamento) {Etapa 4: Treinamento\\dos Modelos};
\node[box, left=of treinamento] (avaliacao) {Etapa 5: Avaliação e\\Rastreabilidade};
...
\subsubsection{Etapa 1: Extração e filtragem do conteúdo}

\textcolor{green}{Nesse parágrao seguinte, tem termos em português (como academia) e inglês (como sports, health). Acho melhor padronizar tudo igual está na base.}

A primeira etapa consistiu em transformar uma base social genérica em um conjunto de dados orientado ao domínio da atividade física. Para isso, o arquivo compactado do \textit{LDBC SNB} foi extraído e seus arquivos \textit{CSV} foram processados. Como a base original contém dados sobre diversos temas não relacionados ao escopo do trabalho, o conteúdo associado à temática \textit{fitness} foi identificado por duas estratégias complementares: correspondência por classes de \textit{tags} (ex: "sports", "health", "fitness") e filtragem direta pelos nomes das próprias \textit{tags} (ex: "running", "workout", "gym"). Esse procedimento resultou em tabelas com mensagens, interações, interesses e relações sociais já restritas ao domínio estudado.
...
Para tornar o processo mais claro, o Algoritmo \ref{alg:extracao} apresenta o pseudocódigo que resume a lógica de alto nível dessa etapa. O procedimento lê o \textit{dataset} bruto, filtra as \textit{tags} relevantes, identifica os \textit{posts} associados e, a partir deles, extrai apenas as interações e as conexões sociais pertinentes ao contexto.

\begin{algorithm}[H]
\caption{Extração e Filtragem de Conteúdo \textit{Fitness}}
\label{alg:extracao}
\textbf{Entrada:} Base bruta do LDBC SNB, Palavras-chave \textit{fitness} (ex: \texttt{running}, \texttt{gym})\\
\textbf{Saída:} Tabelas de \textit{posts}, interações e grafo social filtrados\\
\BlankLine
$tags \gets$ \text{Ler tabela de Tags}\;
$tags\_fitness \gets \emptyset$\;
\For{cada $tag \in tags$}{
    \If{$tag.nome$ contém alguma palavra-chave \textit{fitness}}{
        $tags\_fitness \gets tags\_fitness \cup \{tag\}$\;
    }
}
$posts \gets$ \text{Ler tabela de Posts e Comentários}\;
$posts\_fitness \gets \emptyset$\;
\For{cada $post \in posts$}{
    \If{$post.tags \cap tags\_fitness \neq \emptyset$}{
        $posts\_fitness \gets posts\_fitness \cup \{post\}$\;
    }
}
$interacoes \gets$ \text{Ler tabela de Curtidas, Respostas e Criações}\;
$interacoes\_fitness \gets \emptyset$\;
$usuarios\_ativos \gets \emptyset$\;
\For{cada $interacao \in interacoes$}{
    \If{$interacao.post \in posts\_fitness$}{
        $interacoes\_fitness \gets interacoes\_fitness \cup \{interacao\}$\;
        $usuarios\_ativos \gets usuarios\_ativos \cup \{interacao.usuario\}$\;
    }
}
$grafo\_bruto \gets$ \text{Ler tabela de Relações Sociais (quem conhece quem)}\;
$grafo\_fitness \gets \emptyset$\;
\For{cada $aresta(u, v) \in grafo\_bruto$}{
    \If{$u \in usuarios\_ativos$ \textbf{ou} $v \in usuarios\_ativos$}{
        $grafo\_fitness \gets grafo\_fitness \cup \{aresta(u, v)\}$\;
    }
}
\Return{$posts\_fitness$, $interacoes\_fitness$, $grafo\_fitness$}\;
\end{algorithm}
...
\subsubsection{Etapa 2: Preparação dos dados}

Na segunda etapa, os arquivos extraídos foram convertidos em artefatos próprios para treinamento. Entre eles, destacam-se os metadados semânticos dos posts, a contagem de interações por \textit{tag}, os escores de influência social por post e os perfis usuário-\textit{tag}.

\subsubsection{Etapa 3: Separação experimental}

Na terceira etapa, os dados foram divididos em treino, validação e teste, nas proporções de 70\%, 15\% e 15\%, respectivamente. 
...
\textcolor{green}{Não entendi a explicação do parágrafo a seguir (podemos conversar sobre na próxima reunião):}

Além disso, essa divisão estática foi escolhida em detrimento da Validação Cruzada (\textit{Cross Validation}) para evitar o vazamento de dados (\textit{data leakage}). Em sistemas de recomendação com forte dependência temporal, a ordem em que os eventos ocorrem é fundamental. Atributos como a coocorrência de \textit{tags} (quais assuntos aparecem juntos) e a influência do grafo social (quem interage com quem) representam o estado da rede em um determinado momento. Se a Validação Cruzada clássica fosse aplicada, o modelo poderia utilizar interações que ocorreram no futuro para prever eventos do passado, o que não reflete o cenário real de uso. Portanto, a divisão estática garante que as estatísticas da rede sejam calculadas exclusivamente com dados do conjunto de treino, preservando a validade da avaliação e evitando o custo computacional proibitivo de reconstruir o grafo social a cada iteração (\textit{fold}).

\subsubsection{Etapa 4: Treinamento dos modelos}

Nesta etapa, duas abordagens foram desenvolvidas e treinadas para o sistema de recomendação: um modelo híbrido de referência e uma trilha de aprendizado para ranqueamento.

\paragraph{Modelo híbrido de referência (\textit{Baseline})}
...
Sobre essa representação, o método combina cinco sinais principais: similaridade de conteúdo, coocorrência de \textit{tags}, recência, popularidade e influência do grafo social. Em termos gerais, o \textit{score} final do item é calculado como uma combinação ponderada desses componentes, conforme a Equação \ref{eq:score_hibrido} apresentada na fundamentação teórica, com pesos parametrizáveis conforme o experimento realizado.

\textcolor{green}{Podemos conversar na próxima reunião: acho que esse cálculo do score pode ser apresentado na fundamentação teórica e, nessa parte, somente citar que foi usado esse score}

Para ilustrar como esses sinais são computados, o Algoritmo \ref{alg:treinamento} apresenta o pseudocódigo do treinamento. Diferentemente de redes neurais, o treinamento deste modelo não utiliza otimização por gradiente, mas sim o cálculo de estatísticas sobre o conjunto de treino.

\begin{algorithm}[H]
\caption{Treinamento do Modelo Híbrido (\textit{Baseline})}
\label{alg:treinamento}
\textbf{Entrada:} $posts\_fitness$, $interacoes\_fitness$, $grafo\_fitness$\\
\textbf{Saída:} Artefatos estatísticos serializados para inferência\\
\BlankLine
\tcc{1. Vetorização de Tags (Sem gradiente)}
$vocabulario \gets$ \text{Extrair todas as tags únicas de } $posts\_fitness$\;
$matriz\_posts \gets$ \text{Criar matriz binária (posts $\times$ vocabulario)}\;

\tcc{2. Mapa de Coocorrência de Tags}
$mapa\_coocorrencia \gets$ \text{Dicionário vazio}\;
\For{cada $post \in posts\_fitness$}{
    \For{cada par de tags $(t_a, t_b)$ no $post$}{
        $mapa\_coocorrencia[t_a][t_b] \gets mapa\_coocorrencia[t_a][t_b] + 1$\;
    }
}

\tcc{3. Cálculo de Popularidade}
$popularidade \gets$ \text{Vetor de tamanho } $|posts\_fitness|$\;
\For{cada $post \in posts\_fitness$}{
    $total\_interacoes \gets$ \text{Contar } $interacoes\_fitness$ \text{ associadas às tags do } $post$\;
    $popularidade[post] \gets total\_interacoes$\;
}

\tcc{4. Cálculo de Influência Social}
$grau\_usuarios \gets$ \text{Calcular grau (número de conexões) de cada usuário no } $grafo\_fitness$\;
$score\_social \gets$ \text{Vetor de tamanho } $|posts\_fitness|$\;
\For{cada $post \in posts\_fitness$}{
    $usuarios\_interagiram \gets$ \text{Obter usuários que interagiram com o } $post$\;
    $score\_social[post] \gets \sum_{u \in usuarios\_interagiram} grau\_usuarios[u]$\;
}

\text{Normalizar } $popularidade$ \text{ e } $score\_social$ \text{ para o intervalo } $[0, 1]$\;
\Return{$matriz\_posts$, $mapa\_coocorrencia$, $popularidade$, $score\_social$}\;
\end{algorithm}
...
\paragraph{Trilha de aprendizado para ranqueamento}
...
Sobre esses dados, o \textit{LightGBMRanker} aprende uma função de ordenação a partir de atributos derivados dos mesmos sinais utilizados no modelo híbrido de referência. Assim, em vez de depender apenas de pesos definidos manualmente, a ordem dos conteúdos passa a ser ajustada por um processo supervisionado. Essa trilha foi adotada como base comparativa para verificar em que medida a aprendizagem da ordenação pode superar uma combinação manual e explicável de sinais.

\subsubsection{Etapa 5: Avaliação e rastreabilidade dos experimentos}
...
O Algoritmo \ref{alg:avaliacao} detalha a lógica desse protocolo de avaliação. Ele demonstra como o sistema agrupa as interações por usuário, ordena-as cronologicamente e separa uma interação como referência e as subsequentes como gabarito (\textit{ground truth}), para então calcular as métricas sobre o Top-K gerado pelo modelo.

\begin{algorithm}[H]
\caption{Protocolo de Avaliação \textit{Offline}}
\label{alg:avaliacao}
\textbf{Entrada:} Conjunto de teste ($interacoes\_teste$), Modelo treinado, $K$ (tamanho da lista)\\
\textbf{Saída:} Métricas de avaliação (NDCG@K, MRR@K, etc.)\\
\BlankLine
$resultados \gets \emptyset$\;
$interacoes\_por\_usuario \gets$ \text{Agrupar } $interacoes\_teste$ \text{ por usuário}\;

\For{cada $(usuario, interacoes) \in interacoes\_por\_usuario$}{
    $interacoes\_ordenadas \gets$ \text{Ordenar } $interacoes$ \text{ por tempo}\;
    
    \For{$i \gets 0$ \KwTo $|interacoes\_ordenadas| - 2$}{
        $referencia \gets interacoes\_ordenadas[i]$\;
        $gabarito \gets \{interacoes\_ordenadas[j].post \mid j > i\}$\;
        
        \If{$gabarito \neq \emptyset$}{
            $lista\_recomendada \gets$ \text{Modelo.Recomendar(}$referencia.tags, referencia.tempo, K$\text{)}\;
            
            $acertos \gets lista\_recomendada \cap gabarito$\;
            $ndcg \gets$ \text{Calcular NDCG(}$lista\_recomendada, gabarito$\text{)}\;
            $mrr \gets$ \text{Calcular MRR(}$lista\_recomendada, gabarito$\text{)}\;
            
            $resultados \gets resultados \cup \{(ndcg, mrr, \dots)\}$\;
        }
    }
}
\Return{\text{Média de todas as métricas em } $resultados$}\;
\end{algorithm}
...
Os resultados da primeira rodada, executada na versão \texttt{SF0.1}, estão sintetizados nas tabelas a seguir. Essa rodada funcionou como uma validação inicial do protocolo experimental, operando sobre uma amostra muito reduzida de dados.
...
Para interpretar os resultados, convém explicitar o significado dos valores apresentados. Nas métricas de ranqueamento, o valor 0,0000 indica que, nas 5 consultas válidas do conjunto de teste, nenhum item efetivamente acessado pelo usuário após o item de referência apareceu entre os 10 primeiros recomendados. Em outras palavras, os experimentos foram executados normalmente, mas, nesta rodada inicial, não houve acertos no topo da lista capazes de elevar NDCG@10, MRR@10 e Recall@10 acima de zero. A latência p95, por sua vez, corresponde ao percentil 95 do tempo de inferência, em milissegundos, necessário para gerar a recomendação; portanto, valores menores indicam respostas mais rápidas na maior parte das consultas.

Nos indicadores complementares, a cobertura expressa a fração do catálogo total que apareceu ao menos uma vez nas recomendações ao longo da avaliação. Assim, cobertura 0,9737 significa que aproximadamente 97,37\% do catálogo foi exposto pelo modelo em algum momento. A diversidade, medida numa escala de 0 a 1, representa o grau médio de diferença entre os conjuntos de \textit{tags} dos itens dentro de uma mesma lista; quanto mais próximo de 1, maior a variedade temática interna. A novidade também é expressa em escala crescente e é calculada por popularidade inversa, de modo que valores mais altos indicam recomendação de itens menos frequentes e, portanto, menos óbvios.
...
À luz dessas definições, o modelo \textit{Baseline} padrão foi destacado formalmente como melhor configuração da rodada SF0.1. Ainda assim, esse resultado não deve ser lido como superioridade inequívoca, porque houve empate completo entre os cinco modelos nas métricas centrais de ranqueamento: NDCG@10, MRR@10 e Recall@10 permaneceram em 0,0000 para todas as configurações avaliadas. Com apenas 5 consultas válidas no conjunto de teste, a rodada inicial funcionou mais como validação do protocolo experimental do que como evidência conclusiva de desempenho comparado.

\subsection{Resultados da versão intermediária (SF3)}

Os resultados da rodada intermediária, executada na versão \texttt{SF3}, estão sintetizados nas tabelas a seguir. O aumento do volume de dados em relação à versão inicial gerou um conjunto de teste com 367 consultas válidas, permitindo uma avaliação mais robusta do comportamento dos algoritmos.

\begin{table}[H]
\centering
\caption{Resultados de ranqueamento da rodada intermediária na versão \texttt{SF3}}
\label{tab:resultados_sf3_ranking}
\small
\begin{tabular}{|l|r|r|r|r|r|}
\hline
\textbf{Modelo} & \textbf{Consultas válidas} & \textbf{NDCG@10} & \textbf{MRR@10} & \textbf{Recall@10} & \textbf{Latência p95 (ms)} \\
\hline
\textit{Baseline} padrão & 367 & 0,0000 & 0,0000 & 0,0000 & 4,2467 \\
\hline
\textit{Baseline} otimizado & 367 & 0,0009 & 0,0003 & 0,0027 & 4,3098 \\
\hline
LTR \textit{core} & 367 & 0,0000 & 0,0000 & 0,0000 & 16,2811 \\
\hline
LTR contextual & 367 & 0,0000 & 0,0000 & 0,0000 & 14,5240 \\
\hline
LTR robusto & 367 & 0,0000 & 0,0000 & 0,0000 & 14,6955 \\
\hline
\end{tabular}
\end{table}
...
\begin{table}[H]
\centering
\caption{Indicadores complementares da rodada intermediária na versão \texttt{SF3}}
\label{tab:resultados_sf3_complementares}
\small
\begin{tabular}{|l|r|r|r|r|p{1.5cm}|}
\hline
\textbf{Modelo} & \textbf{Cobertura} & \textbf{Diversidade} & \textbf{Novidade} & \textbf{Recência média (dias)} & \textbf{Avaliação manual} \\
\hline
\textit{Baseline} padrão & 0,3396 & 0,2607 & 0,9478 & 24,90 & N/D \\
\hline
\textit{Baseline} otimizado & 0,6471 & 0,6136 & 0,9323 & 3,18 & N/D \\
\hline
LTR \textit{core} & 0,1678 & 0,8825 & 1,0000 & 399,96 & N/D \\
\hline
LTR contextual & 0,1718 & 0,8689 & 1,0000 & 470,43 & N/D \\
\hline
LTR robusto & 0,1778 & 0,8323 & 1,0000 & 434,70 & N/D \\
\hline
\end{tabular}
\end{table}


Para interpretar as Tabelas \ref{tab:resultados_sf3_ranking} e \ref{tab:resultados_sf3_complementares}, convém explicitar o significado dos valores apresentados. Nas métricas de ranqueamento, o modelo \textit{Baseline} otimizado foi o único a pontuar no top 10 (NDCG@10 de 0,0009), indicando que, entre as 367 consultas válidas, ele conseguiu recuperar itens efetivamente acessados pelo usuário nas primeiras posições. Os demais modelos permaneceram zerados no top 10, embora o \textit{Baseline} padrão tenha pontuado no top 20 (NDCG@20 de 0,0007, conforme registros internos do experimento). Em termos de latência, os modelos híbridos mantiveram respostas muito rápidas (em torno de 4 ms), enquanto as variantes LTR exigiram mais tempo de processamento (cerca de 14 a 16 ms), devido à complexidade da inferência baseada em árvores.

Nos indicadores complementares, o \textit{Baseline} otimizado atingiu uma cobertura de 64,71\%, o dobro do \textit{Baseline} padrão (33,96\%) e muito superior aos modelos LTR (em torno de 17\%). Os modelos LTR geraram listas altamente diversas (acima de 0,83), mas falharam em precisão. Destaca-se também o comportamento da recência: o \textit{Baseline} otimizado teve um desempenho excepcional, recomendando itens com apenas 3,18 dias de distância média do contexto da consulta, enquanto os modelos LTR sugeriram conteúdos com mais de um ano de distância (399 a 470 dias). Esse distanciamento temporal explica, em grande parte, o baixo desempenho do aprendizado para ranqueamento nesta rodada, uma vez que a atualidade é um fator crucial na predição de engajamento em redes sociais.

\noindent\textit{Nota:} A avaliação manual da rodada SF3 foi considerada inconclusiva e, por isso, aparece como N/D na Tabela \ref{tab:resultados_sf3_complementares}.

À luz dessas definições, o modelo \textit{Baseline} otimizado foi destacado formalmente como melhor configuração da rodada SF3. O aumento de dados (de 5 para 367 consultas) tirou os modelos da inércia absoluta observada na versão SF0.1, provando o valor da otimização fina de pesos no conjunto de validação. Contudo, os valores absolutos de NDCG@10 (0,0009) ainda são muito baixos. Isso evidencia a extrema dificuldade de prever interações futuras exatas em um domínio altamente esparso e temporalmente disperso como o da rede social simulada.

Os modelos da família LTR sofreram com a esparsidade dos dados e com a dificuldade de aprender a importância da recência, gerando recomendações muito antigas e diversas, mas pouco precisas. A expectativa é que, com a execução da versão completa da base (SF30), o volume massivo de interações permita que o \textit{LightGBMRanker} encontre padrões mais consistentes e consiga superar a heurística manual do modelo híbrido.

\subsection{Análise comparativa entre versões}

A ampliação progressiva da base, passando da versão SF0.1 para a SF3, permitiu observar as primeiras mudanças no comportamento das métricas à medida que o volume de dados aumenta. A Tabela \ref{tab:comparacao_snapshots_tcc} registra esse acompanhamento evolutivo.
...
\small
\begin{tabular}{|l|l|r|r|r|p{3.5cm}|}
\hline
\textbf{Rodada} & \textbf{Versão} & \textbf{Modelo destacado} & \textbf{NDCG@10} & \textbf{Cobertura} & \textbf{Observação} \\
\hline
Inicial & \texttt{SF0.1} & \textit{Baseline} padrão & 0,0000 & 0,9737 & Empate em zero; amostra de apenas 5 consultas válidas. \\
\hline
Intermediária & \texttt{SF3} & \textit{Baseline} otimizado & 0,0009 & 0,6471 & Aumento para 367 consultas; Baseline otimizado sai da inércia. LTR sofre com esparsidade. \\
\hline
...
A transição da versão SF0.1 para a SF3 revelou que o aumento de dados é fundamental para retirar os modelos da inércia absoluta (NDCG zerado). Com 367 consultas válidas, o \textit{Baseline} otimizado conseguiu demonstrar a eficácia do ajuste fino de pesos. No entanto, os valores absolutos de NDCG@10 (0,0009) ainda permanecem muito baixos. Essa dificuldade inerente reflete a natureza do domínio: prever interações futuras exatas em uma rede social simulada é uma tarefa de alta esparsidade e dispersão temporal.

Além disso, a comparação evidencia o desafio enfrentado pelos modelos de aprendizado para ranqueamento (\textit{Learning to Rank}). Na rodada SF3, as variantes LTR apresentaram latência significativamente maior e não conseguiram superar a precisão ou a recência da heurística híbrida. Isso sugere que a quantidade de dados da SF3 ainda não foi suficiente para que as árvores de decisão aprendessem relações complexas de forma generalizável. 

A expectativa para a rodada final, com a versão completa da base (SF30), é que o volume massivo de interações permita que o \textit{LightGBMRanker} encontre padrões consistentes, reduzindo a distância temporal de suas recomendações e, potencialmente, superando o modelo híbrido de referência.

\textit{[Nota para a versão final: Inserir aqui a identificação do melhor modelo final segundo NDCG@10 na base SF30, seus parâmetros principais e a comparação quantitativa definitiva entre a família híbrida e as variantes LTR.]}
