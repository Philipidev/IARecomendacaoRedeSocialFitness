...
Essa divisão estática foi escolhida em detrimento da Validação Cruzada (\textit{Cross Validation}) para evitar o vazamento de dados (\textit{data leakage}). No contexto deste sistema, atributos complexos como a coocorrência de \textit{tags} e os escores de influência do grafo social precisam ser recalculados \textbf{exclusivamente} com os dados do conjunto de treino. Se a Validação Cruzada fosse aplicada, o custo computacional de reconstruir e recalcular o grafo social a cada \textit{fold} seria proibitivo, além de introduzir o risco de que interações futuras (do conjunto de teste) influenciassem os pesos aprendidos no treinamento.
